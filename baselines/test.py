...
class MPC_Policy_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.gelu(self.linear6(x))
        x = F.tanh(self.linear7(x))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ep", type=int, nargs='?', default=10000) # the same as cdpc
    parser.add_argument("--b", type=int, nargs='?', default=128) # the same as cdpc
    parser.add_argument("--lr", type=float, nargs='?', default=1e-3) # the same as cdpc
    ...
    if args.wandb:
        wandb.init(project="cdpc", name = f'baseline: BC lr={args.lr} batch={args.b}')
    ...
    buffer = collect_target_data(trained_agent, env, 10, device, seed)
